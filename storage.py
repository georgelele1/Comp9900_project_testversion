...
PROVIDER_OPTIONS: List[Dict[str, Any]] = [
    {
        ...
        "key_prefixes": ["AIza"],
        "models": [
            {"id": "co/gemini-3-flash-preview", "label": "Gemini 3 Flash"},
            {"id": "co/gemini-3-pro-preview", "label": "Gemini 3 Pro"},
            {"id": "co/gemini-2.5-flash", "label": "Gemini 2.5 Flash"},
        ],
    },
    {
        "id": "openai",
        ...
        "key_prefixes": ["sk-"],
        "models": [
            {"id": "gpt-5.4", "label": "GPT-5.4 Fast"},
            {"id": "gpt-5", "label": "GPT-5 Powerful"},
            {"id": "gpt-4o", "label": "GPT-4o Efficient"},
        ],
    },
    {
        "id": "anthropic",
...
def snippets_path() -> Path:
    return storage_path(SNIPPETS_FILE)


def session_path() -> Path:
    return storage_path(SESSION_FILE)
...
def get_target_language() -> str:
    lang = load_profile().get("preferences", {}).get(
        "target_language",
        DEFAULT_LANGUAGE,
    )
...
def apply_dictionary_corrections(text: str) -> str:
    ...
    for item in load_dictionary().get("terms", []):
        ...
        for alias in aliases:
            ...
            if re.fullmatch(r"[A-Za-z0-9_][A-Za-z0-9_\- ]*[A-Za-z0-9_]", alias):
...
def clear_snippets() -> None:
    save_store(SNIPPETS_FILE, {"snippets": []})


def clear_session_file() -> None:
    save_store(SESSION_FILE, {"updated_at": time.time(), "messages": []})
