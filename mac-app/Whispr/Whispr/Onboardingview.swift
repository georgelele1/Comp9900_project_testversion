...
// OnboardingView
//
// Single-page first-launch setup.
// No name/email — just pick usage habits and writing style.
// Everything is optional; "Skip" marks onboarding done too.
// =========================================================
                ...
                ScrollView(showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 22) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            LazyVGrid(columns: cols, spacing: 10) {
                                ForEach(usageOptions) { opt in
                                    OnbPill(option: opt,
                                            selected: selectedUsage.contains(opt.label),
                                            accent: accent) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            let cols2 = [GridItem(.flexible(), spacing: 10),
                                         GridItem(.flexible(), spacing: 10)]
                            LazyVGrid(columns: cols2, spacing: 10) {
                                ForEach(interestOptions) { opt in
                                    OnbPill(option: opt,
                                            selected: selectedInterests.contains(opt.label),
                                            accent: accent) {
                        ...
                        // Writing style + language in one row
        ...
        Button(action: action) {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 10)
                    .stroke(selected ? accent : Color.secondary.opacity(0.2),
